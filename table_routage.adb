...
new Ada.Unchecked_Deallocation (Object => T_Cellule, Name => T_Table_Routage);
        ...
        begin
                while Table_parcours /= Null loop
                        if Destination /= Table_parcours.all.Destination and then Comp_Destination_Paquet(Destination, Masque, Table_parcours.all.Destination) and then Masque_coherent < Table_parcours.all.Masque then
                                Ie_bit := Trouver_Ie_Bit_Diff(Destination_correspondante, Table_parcours.all.Destination);
