...
package body Cache_A is
   ...
   --begin
     ...
     -- if Est_Vide(Cache) then
      --   Cache:= new T_Cellule'(Destination,Masque,Interface_eth,null,null);
     --- else
     --    if (Dest_Copie and POIDS_FORT) /=0 then
     --      if not(Est_Feuille(Cache)) then
       --        Ajouter_C(cache.all.Suivant_D,Destination,Masque,Interface_eth);
      --         Dest_Copie := Dest_Copie*2;
       --     else
         --      cache.all.Suivant_D:=null;
           --    Ajouter_C(cache.all.Suivant_D,Destination,Masque,Interface_eth);
--            end if;
  --       else
    --        if not(Est_Feuille(Cache)) then
      --         Ajouter_C(cache.all.Suivant_G,Destination,Masque,Interface_eth);
        --       Dest_Copie := Dest_Copie*2;
          --  else
            --   cache.all.Suivant_G:=null;
              -- Ajouter_C(cache.all.Suivant_G,Destination,Masque,Interface_eth);
--            end if;
  --       end if;
  ...
  -- end Ajouter_C;
     ...
     --    Cache:= new T_Cellule'(Destination,Masque,Interface_eth,null,null);
      ...
      --else
         --if Ie_Bit_A_1(Destination,i) and Ie_Bit_A_1(Cache.all.Destination,i) then
         --  Ajouter_C(Cache.all.Suivant_D,1,0,Interface_eth,i);
         --   i:=i+1;
         --elsif not(Ie_Bit_A_1(Destination,i)) and not(Ie_Bit_A_1(Cache.all.Destination,i)) then
         --   Ajouter_C(Cache.all.Suivant_G,0,0,Interface_eth,i);
         --   i:=i+1;
         -- else
        -- if not(Est_Feuille(Cache)) then
        --    if Ie_Bit_A_1(Destination,i) then
          ...
          --  else
           ...
           -- end if;
         --else
           -- if Destination < Cache.all.Destination then
              -- if Ie_Bit_A_1(Destination,i) then
                 -- Cache.all.Suivant_D:= null;
                --  Ajouter_C(Cache.all.Suivant_G,Destination,Masque,Interface_eth,i);
                --  Ajouter_C(Cache.all.Suivant_D,Cache.all.Destination,Cache.all.Masque,Cache.all.Interface_eth,i);
              -- else
                --  Cache.all.Suivant_G:= null;
                 -- Ajouter_C(Cache.all.Suivant_G,Destination,Masque,Interface_eth,i);
               --   Ajouter_C(Cache.all.Suivant_D,Cache.all.Destination,Cache.all.Masque,Cache.all.Interface_eth,i);
           ...
           -- else
             --  if Ie_Bit_A_1(Destination,i) then
                 -- Cache.all.Suivant_D:= null;
                 -- Ajouter_C(Cache.all.Suivant_D,Destination,Masque,Interface_eth,i);
                 -- Ajouter_C(Cache.all.Suivant_G,Cache.all.Destination,Cache.all.Masque,Cache.all.Interface_eth,i);
               --else
                 -- Cache.all.Suivant_G:=null;
                 -- Ajouter_C(Cache.all.Suivant_D,Destination,Masque,Interface_eth,i);
               --   Ajouter_C(Cache.all.Suivant_G,Cache.all.Destination,Cache.all.Masque,Cache.all.Interface_eth,i);
     ...
     begin
          if Est_Vide(Cache) then
               ...
               if (Cache.all.Suivant_G=null and Cache.all.Suivant_D=null) and (Cache.all.Destination = Destination) then
                    Cache.all.Destination:= Destination;
                    Cache.all.Masque:=Masque;  
                    Cache.all.Interface_eth:=Interface_eth;
               
               elsif (Cache.all.Suivant_G=null and Cache.all.Suivant_D=null) and not(Cache.all.Destination = Destination) then

                    Noeud_Copie:= new T_Cellule;
                    if Cache.all.Destination = Destination and Ie_Bit_A_1(Destination, i) then
               ...
               elsif (Cache.all.Suivant_G=null and Cache.all.Suivant_D=null) and Ie_Bit_A_1(Destination, i) then
                    i:=i+1;
                    Ajouter_C(Cache.all.Suivant_D,Destination,Masque,Interface_eth,i);
